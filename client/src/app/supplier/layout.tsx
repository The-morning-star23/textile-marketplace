...
export default function SupplierLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#080C17] flex font-sans text-cyan-50 selection:bg-cyan-500/30">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-[#0B1120]/80 border-r border-indigo-500/20 backdrop-blur-xl shrink-0 z-50">
        ...
        <div className="p-6">
          ...
          <nav className="space-y-2">
            {supplierLinks.map((link) => {
              // Check if current path matches OR if we are on the 'new' product page
        ...
        <div className="mt-auto p-6 border-t border-indigo-500/20">
          <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3.5 w-full rounded-xl font-bold text-red-400 border border-transparent hover:bg-red-500/10 hover:border-red-500/30 transition-all">
